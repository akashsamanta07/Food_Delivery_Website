        ...
        <div className='w-[95%] md:w-[70%] mx-auto mt-[3rem] pb-[3rem] flex flex-wrap gap-[0.8rem] md:gap-[1.25rem] justify-center'>
            {
              filter_food_items.map((item,i)=>{
                return (
                  <Card 
                    id={item.id} food_name= {item.food_name} food_category= {item.food_category} food_type= {item.food_type} food_quantity ={item.food_quantity} food_image= {item.food_image} price= {item.price}
                  key={i}/>
                )
              })
            }
